...
if __name__ == '__main__':
    ...
    for cnt in contours:
        ...
        if area < maxArea/10:
            ...
            eroded_shape = cv2.erode(cropped.copy(), kernel, iterations=1)
            dilate_shape = cv2.dilate(eroded_shape.copy(), kernel, iterations=1)
            img[y - margin:y + h + margin,x - margin:x + w + margin] = dilate_shape  # we copy the eroded_shape back into the original_image
